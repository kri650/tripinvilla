...
import { Search, Filter, Calendar, ChevronDown, ClipboardList, Clock, XCircle, CheckCircle2, MoreVertical, Plus, X, Upload } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const newId = createdProp.id || createdProp._id;
        ...
        alert(d.message || 'Failed to add property');
      }
    } catch (err) {
      console.error(err);
      ...
      {showPanel && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9000, display: 'flex' }}>
          ...
          <div style={{ width: 680, background: '#fff', height: '100vh', overflowY: 'auto', boxShadow: '-8px 0 40px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column' }}>
            ...
            {/* Panel Form */}
            <form onSubmit={handleSubmit} className="master-form-card" style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 20, flex: 1, boxShadow: 'none', background: 'transparent', margin: 0 }}>
              
               {/* Row 0: Assign to Owner */}
              <div className="form-group">
                <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>Assign to Owner*</label>
                <select className="form-select" name="ownerId" required value={form.ownerId} onChange={handleFormChange}>
              ...
              <div className="form-grid-3">
                ...
                <div className="form-group">
                  ...
                  <input type="text" className="form-input" name="name" value={form.name} onChange={handleFormChange} placeholder="Enter property name" required />
                </div>
                <div className="form-group">
                  <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>Room Type*</label>
                  <input type="text" className="form-input" name="roomType" value={form.roomType} onChange={handleFormChange} placeholder="e.g. 1 Deluxe 4 Normal" required />
                </div>
              </div>
              ...
              <div className="form-grid-3">
                <div className="form-group">
                  ...
                  <input type="text" className="form-input" name="ownerContact" value={form.ownerContact} onChange={handleFormChange} placeholder="Owner contact number" required />
                </div>
                <div className="form-group" style={{ gridColumn: 'span 2' }}>
                  <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>
                    ...
                    <span style={{ marginLeft: 8, fontSize: 11, color: '#9CA3AF', fontWeight: 400 }}>
                      Showing amenities for: <strong style={{ color: '#58A429' }}>{form.type}</strong>
                    </span>
                  </label>
                  {amenitiesLoading ? (
              ...
              <div className="form-group" style={{ border: '1px solid #E5E7EB', padding: '16px', borderRadius: '8px', background: '#F9FAFB' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                  <div style={{ gridColumn: 'span 2' }}>
                    ...
                    <input type="text" className="form-input" name="full_address" value={form.full_address} onChange={(e) => {
                      handleFormChange(e);
                      setForm(prev => ({ ...prev, location: e.target.value, full_address: e.target.value }));
                  ...
                  <div>
                    ...
                    <input type="number" step="any" className="form-input" name="latitude" value={form.latitude} onChange={handleFormChange} placeholder="e.g. 32.0100" />
                  </div>
                  <div>
                    <label className="form-label" style={{ fontSize: '12px', color: '#4B5563' }}>Longitude</label>
                    <input type="number" step="any" className="form-input" name="longitude" value={form.longitude} onChange={handleFormChange} placeholder="e.g. 77.2970" />
                  </div>
                </div>
                
                {form.latitude && form.longitude && (
                  <div style={{ width: '100%', height: '200px', borderRadius: '8px', overflow: 'hidden', border: '1px solid #D1D5DB', marginBottom: 12 }}>
                    <iframe
              ...
              <div className="form-grid-3">
                <div className="form-group">
                  ...
                  <input type="number" className="form-input" name="price" value={form.price} onChange={handleFormChange} placeholder="₹ Amount" required />
                </div>
                <div className="form-group">
                  ...
                  {(existingImages.length > 0 || selectedFiles.length > 0) && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '10px' }}>
                      {existingImages.map((url, idx) => (
                        <div key={`ex-${idx}`} style={{ position: 'relative', width: '60px', height: '60px' }}>
                  ...
                  {(existingImages.length + selectedFiles.length) < 10 && (
                    <div className="file-upload-wrapper" onClick={() => fileInputRef.current.click()} style={{ cursor: 'pointer', display: 'flex', border: '1px solid #D1D5DB', borderRadius: '8px', overflow: 'hidden' }}>
                      <input type="text" className="file-upload-input" style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '13px' }} value={existingImages.length + selectedFiles.length > 0 ? `${existingImages.length + selectedFiles.length} image(s)` : 'Browse images'} readOnly />
                      <button type="button" style={{ background: '#58A429', color: '#fff', border: 'none', padding: '8px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Upload size={14} /></button>
              ...
              <div className="form-grid-3">
                <div className="form-group">
                  ...
                  <input type="text" className="form-input" name="checkIn" value={form.checkIn} onChange={handleFormChange} placeholder="e.g. 3:00 PM" required />
                </div>
                <div className="form-group">
                  <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>Check-Out Time*</label>
                  <input type="text" className="form-input" name="checkOut" value={form.checkOut} onChange={handleFormChange} placeholder="e.g. 12:00 PM" required />
                </div>
                <div className="form-group">
                  <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>Area Size*</label>
                  <input type="text" className="form-input" name="area" value={form.area} onChange={handleFormChange} placeholder="e.g. 31 sq. ft." required />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
                <div className="form-group">
                  ...
                  <input type="number" className="form-input" name="bedRooms" value={form.bedRooms} onChange={handleFormChange} placeholder="Rooms" required />
                </div>
                <div className="form-group">
                  <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>Beds Count*</label>
                  <input type="number" className="form-input" name="beds" value={form.beds} onChange={handleFormChange} placeholder="Beds" required />
                </div>
                <div className="form-group">
                  <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>Guests Capacity*</label>
                  <input type="number" className="form-input" name="capacity" value={form.capacity} onChange={handleFormChange} placeholder="Guests" required />
                </div>
                <div className="form-group">
                  <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>Bath Rooms*</label>
                  <input type="number" className="form-input" name="bathRooms" value={form.bathRooms} onChange={handleFormChange} placeholder="Baths" required />
                </div>
              </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {landmarksList.map((lm, idx) => (
                    <div key={idx} style={{ 
                      ...
                      <button type="button" onClick={() => handleRemoveLandmark(idx)} style={{ background: 'none', border: 'none', color: '#EF4444', cursor: 'pointer', padding: 0, display: 'flex', marginLeft: '2px' }}>
              ...
              <div className="form-group">
                ...
                <textarea className="form-textarea" name="rules" rows={3} value={form.rules} onChange={handleFormChange} placeholder="e.g. • Primary Guest should be atleast 18 years of age." required />
              </div>

              {/* Row 9: Description / About */}
              <div className="form-group">
                <label className="form-label" style={{ fontFamily: '"Outfit", sans-serif' }}>About Property*</label>
                <textarea className="form-textarea" name="description" rows={3} value={form.description} onChange={handleFormChange} placeholder="Write description about property..." required />
              </div>
